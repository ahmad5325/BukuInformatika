...
class Kepo(object):
	...
	def cukupHalaman(self,userrepo,namarepo,namafile,pertemuan):
		...
		if "404" in str(response):
			print("link salah, perhatikan case sensitive!")

		pdf_io = io.BytesIO(response.content)
		maca_pdf = PdfFileReader(pdf_io)

		halaman = maca_pdf.getNumPages() / 50

		if halaman >= int(pertemuan):
			return True
		else:
			return False
	...
	def generateURL(self, NPM, PASSWORD, PROYEK, NILAI, KET, Pembimbing,userrepo,namarepo,namafile,pertemuan):
		lengkap=self.sudahLengkap(NPM, PASSWORD, PROYEK, NILAI, KET, Pembimbing,userrepo,namarepo,namafile,pertemuan)
	...
	def sudahLengkap(self, NPM, PASSWORD, PROYEK, NILAI, KET, Pembimbing,userrepo,namarepo,namafile,pertemuan):
		...
		cukuphalaman=self.cukupHalaman(userrepo,namarepo,namafile,pertemuan)
		if not cukuphalaman:
			print('Halaman masih kurang')
		if adafoto and adaosjur and kodedosenbenar and cukuphalaman:
			return True
		else:
			return False
